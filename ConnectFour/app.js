function runGame(){
    ...
    for (let i = 0; i<ships.length; i++){
        ships[i].onclick = () => {
            if (ships[i + 7].classList.contains("taken") && !ships[i].classList.contains("taken")) {
                if (currentPlayer.textContent == 1){
                    ships[i].classList.add("taken");
                    ships[i].classList.add("player-one");
                    currentPlayer.textContent = 2;
                } else if (currentPlayer.textContent == 2){
                    ships[i].classList.add("taken");
                    ships[i].classList.add("player-two");
    ...
    function checkBoard() {
        ...
        for (let y = 0; y<winningArrays.length; y++){
            const square1 = ships[winningArrays[y][0]];
            const square2 = ships[winningArrays[y][1]];
            const square3 = ships[winningArrays[y][2]];
            const square4 = ships[winningArrays[y][3]];
